...
async function GetSimilarGithubCommentId(pullRequestComments) {
    ...
    if (pullRequestComments.length != 0) {
        for (let comment of pullRequestComments) {
            ...
            if(similarTaskCount === inputs.commentTaskList.split(";").length - 1 && comment.body.includes(inputs.title)) {
...
async function clearPreviousComments() {
    for (let [key,value] of mapCommentSimilar) {
        if (value > 0 && inputs.deletePreviousComments) {
