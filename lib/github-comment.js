...
function getSimilarGithubCommentId(pullRequestComments) {
    ...
    if (pullRequestComments.length != 0) {
        var taskListMatches = pullRequestComments.matchAll(inputs.commentTaskList);
        for (let comment of pullRequestComments) {
            if (comment.body.includes(inputs.body) && comment.body.includes(inputs.title) || taskListMatches > 0) {
