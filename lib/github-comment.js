...
async function initializeComment() {
    ...
    if (!inputs.commentPrefixId) {
        core.setFailed("The `comment-differentiator-prefix` input needs to be set");
    }

    if (inputs.showCommentId) {
        resultComment += "> Comment Prefix -> `" + inputs.commentPrefixId + "` \n";
    }
...
async function createGithubComment(commentBody) {
    ...
    if (commentPrefixIDs.includes(comment.id)) {
        core.setFailed(`There is already a task list comment with the same comment-id value: ${comment.id}`);
    }

    commentPrefixIDs.push(comment.id);
...
function getSimilarGithubCommentId(pullRequestComments) {
    ...
    if (pullRequestComments.length != 0) {
        for (let comment of pullRequestComments) {
            if (commentPrefixIDs.includes(inputs.commentPrefixId)) {
