...
async function getSimilarGithubCommentId(pullRequestComments) {
    ...
    var differentCommentsCount = 0;

    if (pullRequestComments.length != 0) {
        for (let comment of pullRequestComments) {
            var differentTaskCount = 0;
            for (task of inputs.commentTaskList.trim().split(";")) {
                if (comment.body.trim().includes(task) || task.includes(comment.body.trim()) && !similarCommentIds.includes(comment.id)) {
            ...
            if(differentTaskCount > 0) {
                differentCommentsCount++;
            } else {
    ...
    if (similarCommentIds.length > 1) {
        ...
        for (let commentID of similarCommentIds) {
            if(commentID !== similarCommentId) {
                console.log(`Deleting the comment with ID: ${commentID}`);
                await deleteGithubComment(commentID);
            }
        }
    }

    if(differentCommentsCount > 0) {
