...
async function runTimer(commentId) {
    ...
    const count = await getTaskListCount(commentId, CHECK_LIST_REGEX);
    ...
    var interval = setInterval(async function() {
        ...
        if (sec < 0 || completedTasksArr.length == count && count != 0) {
            ...
            const completionTime = sec > 10000 ? sec/60 : sec; 

            console.log(`🎉 You've succesfully completed all ${completedTasksArr.length} tasks in ${completionTime} seconds 🎉`);
